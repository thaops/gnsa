...
import 'package:gnsa/feature/presentation/flight_list/presentation/provider/providers.dart';
...
class FlightListNotifier extends _$FlightListNotifier {
  ...
  Future<FlightsModel> build(bool isMyFlight) async {
    ...
    // Set mặc định ngày: 7 ngày gần nhất nếu chưa có
    if (_fromDate == null || _toDate == null) {
      _toDate = DateTime.now();
      _fromDate = _toDate!.subtract(const Duration(days: 7));
    }
    
    final result = await _fetchFlights(
      search: null,
      fromDate: _fromDate,
      toDate: _toDate,
      cancelToken: _cancelToken!,
  ...
  Future<void> loadMore({String? search}) async {
    ...
    try {
      final moreFlights = await _fetchFlights(
        search: search,
        fromDate: _fromDate,
        toDate: _toDate,
        cancelToken: _cancelToken!,
  ...
  Future<void> refreshFlights(
    ...
    // Nếu không truyền ngày và chưa có ngày nào được set, set mặc định 7 ngày gần nhất
    if (fromDate == null && toDate == null && (_fromDate == null || _toDate == null)) {
